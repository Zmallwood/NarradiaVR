...
namespace nar {
    ...
    void GraphicsGLView::InitializeDevice(XrInstance instance, XrSystemId system_id) {
        ...
        if (!ksGpuWindow_Create(
                &window_, &driver_instance, &queue_info, 0, color_format, depth_format,
                sample_count, 640, 480, false)) {
    ...
    void GraphicsGLView::InitializeResources() {
        ...
        glVertexAttribPointer(
            vertex_attrib_coords_, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), nullptr);
        glVertexAttribPointer(
    ...
    GraphicsGLView::SelectColorSwapchainFormat(const std::vector<int64_t> &runtime_formats) const {
        ...
        auto swapchain_format_it = std::find_first_of(
            runtime_formats.begin(), runtime_formats.end(),
            supported_color_swapchain_formats.begin(), supported_color_swapchain_formats.end());
...
//         XrMatrix4x4f_CreateTranslationRotationScale(&to_view, &pose.position, &result, &scale);
	...
	t.z += player_translation.z;
       XrMatrix4x4f_CreateTranslationRotationScale(&to_view, &t, &result, &scale);
        ...
        //    // Draw the cube.
        //    glDrawElements(
        //        GL_TRIANGLES, static_cast<GLsizei>(vertex_cube.indices().size()),
