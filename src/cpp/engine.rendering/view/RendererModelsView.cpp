...
#include "ShaderSourceRendererModelsFragment.inc.cpp"
   ...
   RenderId
   RendererModelsView::NewBodyKeyframe(std::string_view model_name, float msTime, int numVertices) {
   ...
   void RendererModelsView::NewBodyKeyframeGeometry(
      ...
      //      value_ptr(CameraGl::Get()->GetViewMatrix())); glUniform1f(location_alpha_, 1.0f); auto
      //      image_id = ImageBank::Get()->GetImage(model_name); glBindTexture(GL_TEXTURE_2D,
      //      image_id);
   ...
   void RendererModelsView::DrawModel(
      ...
      if (p_model->animDuration == 0)
         ...
         ms_time_used = static_cast<int>(msTime * global_animation_speed_) % p_model->animDuration;
