...
#include "../../engine.model_structure/model/ModelPart.h"
...
namespace nar {
   auto ModelPartCreator::CreateModelPartFromMesh(
      ...
      auto tex_name_hash = GetTexNameForMesh(scene, mesh);
      new_model_part->texName = tex_name_hash;
   ...
   std::string ModelPartCreator::GetTexNameForMesh(const aiScene *scene, aiMesh *mesh) const {
   ...
   vector<std::string> ModelPartCreator::GetTexNames(const aiScene *scene) const {
