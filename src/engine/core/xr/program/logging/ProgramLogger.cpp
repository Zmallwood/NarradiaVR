...
namespace nar {
  ...
  void ProgramLogger::LogViewConfigurations() {
    ...
    Log::Write(
        Log::Level::Info, Fmt("Available View Configuration Types: (%d)", view_config_type_count));
  ...
  void ProgramLogger::LogEnvironmentBlendMode(XrViewConfigurationType type) {
    ...
    for (XrEnvironmentBlendMode mode : blend_modes) {
      const bool blend_mode_match = (mode == OptionsManager::Get()->options()->Parsed.environment_blend_mode);
      Log::Write(
  ...
  void ProgramLogger::LogActionSourceName(XrAction action, const std::string &action_name) const {
    ...
    for (uint32_t i = 0; i < path_count; ++i) {
      ...
      XrInputSourceLocalizedNameGetInfo name_info = {XR_TYPE_INPUT_SOURCE_LOCALIZED_NAME_GET_INFO};
  ...
  void ProgramLogger::LogLayersAndExtensions() {
    ...
    // Log layers and any of their extensions.
    {
      ...
      for (const XrApiLayerProperties &layer : layers) {
        Log::Write(
            Log::Level::Verbose, Fmt("  Name=%s SpecVersion=%s LayerVersion=%d Description=%s",
